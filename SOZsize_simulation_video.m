...
for i=1:length(r_SOZs)
    tic
    subplot(10,1,1:9) %clear all subplots
    % low density (LD)                          % (dims,r,        iedist,   XYZshift,sp,   color_s,  color_e, newfig,brainplot,F)
    ...
    hold on
    grid on
    %plot(s_vol_LD(1:i),e_vol_LD(1:i),'.','markersize',15,'color',color_LD*.7) %estimate SOZ with LD
    %set(gca,'XScale','log','YScale','log')
    plot(s_vol_LD(1:i),e_vol_HD(1:i),'.-','linewidth',3,'color',color_HD*.7) %estimate SOZ with HD
    %plot(s_vol_HD(1:i),e_vol_HD(1:i),'.','markersize',15,'color',color_HD*.7) %estimate SOZ with HD
    %lines
    %xlim([50e2 1.6e4])
    ...
    subplot(4,3,12)
    %dots
    %plot(s_vol_LD(1:i),e_vol_HD(1:i)-e_vol_LD(1:i),'.-','linewidth',4,'color',mean([color_LD;color_HD],1)*.8) %estimate SOZ with LD
    ...
    %plot(s_vol_LD(1:i),e_vol_HD(1:i)-e_vol_LD(1:i),'-','markersize',15,'color',mean([color_LD;color_HD],1)*.8) %estimate SOZ with LD
    %set(gca,'XScale','log','YScale','log')
    %title('HD minus LD (mm^3)')
...
vidfn = ['~/Desktop/OPSCEAhilo_sim_vids/' jkdatetime];
